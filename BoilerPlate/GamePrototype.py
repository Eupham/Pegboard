...
def ai_move(color, vector):
    r, theta = vector
    valid_spaces = load_valid_spaces()
    nearest_space = find_nearest_valid_space(r, theta, valid_spaces)

    # Plot the move on both charts
    ax1.plot(nearest_space[1], nearest_space[0], marker='o', markersize=5, color=color)
    ax2.plot(nearest_space[1], nearest_space[0], marker='o', markersize=5, color=color)
    fig.canvas.draw()
...
move_data = {"color": "red", "address": [1.0, 0.5235987755982988]}
delay = random.uniform(1, 2)
time.sleep(delay)
ai_move(move_data["color"], tuple(move_data["address"]))
plt.show()
# Test the ai_move function with the provided data
